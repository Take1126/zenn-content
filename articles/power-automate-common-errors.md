...
1. **フィルタークエリの構文を確認する**
   ```
   ❌ statuscode　eq　1   （※全角スペースが混入している例。見た目では判別しにくい）
   ✅ statuscode eq 1      （半角スペースを使用）
   ```
   ※ 全角スペースは使用不可です。コピー＆ペースト時に全角スペースが混入するケースが多いため注意してください。
...
- **関数名のタイポや括弧の閉じ忘れ**（例: `substring` を `substirng` と書く、括弧の対応が合っていないなど）
...
2. **Apply to each の並列度を調整する**
   Apply to each の設定 → 「コンカレンシー制御」をオンにし、並列度を下げることでスロットリングを回避できます。`1` にすれば確実ですが処理速度が大幅に低下するため、処理量に応じて最適な並列数（5〜20 程度）を探るのがおすすめです。
